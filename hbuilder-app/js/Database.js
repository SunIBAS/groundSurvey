...
const tables = {
    image: 'create table if not exists image(id varchar(20) PRIMARY KEY not null,imageUrl varchar (256) not null,imgBase64 text)',
    ...
    executeSql(sql) {
        return new Promise((s,f) => {
            plus.sqlite.executeSql({
                ...
                fail: function(e){
                    console.group("error api executeSql");
                    console.log(`sql = ${sql}`);
                    console.log(`error = ${e}`);
                    console.groupEnd();
                    f(e);
...
class RecordApi extends Api {
    static ArrayMap = {
        disease: 'diseaseImgRecordList',
        drought: 'droughtImgRecordList',
        pest: 'pestImgRecordList'
    }
    constructor() {
        super("record");
    ...
    getRecord({id}) {
        return this.selectSql(id).then(e => {
            if (e.length === 1) {
                e = {
                    ...e[0],
                    diseaseImgRecordList: [],
                ...
                return this.imageRecordApi.selectByRecordId(e.id).then(arrs => {
                    arrs.forEach(a => {
                        e[RecordApi.ArrayMap[a.imageType]].push({
                            id: a.id,
                            imgUrl: `img-db:${a.imageId}`,
                            recordId: e.id,
                            createTime: '',
                            updateTime: ''
                        });
...
class ImageApi extends Api {
    ...
    createImage({imgBase64}) {
        // id,imageUrl
        let id = createId();
        return FileStorage.savebase64ToBitmap(imgBase64,'_doc/img/' + id + '.jpg').then(path => {
            let imageUrl = `img-db:${path}`;
    ...
    getImage({id}) {
        return this.selectSql(id).then(obj => {
            // return obj[0].imgBase64;
            let path_ = obj[0].imageUrl;
            if (path_.startsWith('img-db:')) {
                path_ = path_.substring('img-db:'.length);
            }
            return FileStorage.readBitmapToBase64(path_);
        });
    }
    deleteImage({imageId}) {
...
class ImageRecordApi extends Api {
    ...
    selectByRecordId(recordId) {
        return this.selectSqlExecute(`select * from ${this.tbname} where recordId="${recordId}"`);
    }
}
